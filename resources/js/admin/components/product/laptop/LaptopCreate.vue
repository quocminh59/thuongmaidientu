		...
		<simplebar data-simplebar-auto-hide="true" class="laptop-create">
			<form @submit.prevent="save()" >
				<div class="row">
					<simplebar data-simplebar-auto-hide="true" class="area-left col-sm-6">
						...
						<div class="row mb-4">
							...
							<div class="col-sm-9">
								<input-tag id="product-tag"  :tagPassed="[]" :directive="directive" @passing="product.tag = $event" class="form-control para"></input-tag>
							</div>
						</div>
					...
					<simplebar class="area-right col-sm-6" data-simplebar-auto-hide="true">
						...
						<div class="row mb-4">
							...
							<div class="col-sm-8">
								<input type="text" v-model="product.para.harddisk" id="hard-disk" class="form-control para">
							</div>
						</div>
				...
				<div class="row mb-4">
					...
					<div class="col-sm-12">
						<select multiple class="form-control" id="parent-id" v-model="product.category">
...
export default {
	...
	data() {
  	...
  	methods: {
		...
		multiUploadImage() {		
			...
			console.log(this.arrayMultiImage);	
		},
		...
		save() {
			...
			axios.post(`${base}/admin/product/create`, this.product)
			.then(()=> {
				this.show = false;
				this.display = true;
				this.message.type = "success";
				this.message.content = "Thêm sản phẩm thành công";
				this.resetValue();
			}).catch((error)=> {
				this.show = true;
                ...
                this.message.content = "Thêm sản phẩm thất bại";
			})
		...
		resetValue: function() {
			...
			this.resetValueCKEDITOR('content-laptop');
			this.resetValueCKEDITOR('gift-laptop');
			this.directive = 'stop';
			setTimeout(()=> {
				this.directive = 'pass';
			},2000)
	...
	created () {
		axios.get(`${base}/admin/category/recursive/showsub/1`).then((response)=> {
